...
class LRP(LinearBandit):
    ...
    def init_run(self, n):
        ...
        ## Init D0
        D = np.zeros(self.d)
        indices_D = self.rng.choice(self.d, self.q)
        for i in indices_D:
            D[i] = np.sqrt(self.m/self.q)
        self.D0 = np.diag(D)
        ## Init P0
        self.P0 = self.rng.normal(0, np.sqrt(1 / self.m), size = (self.m, self.d))  
        ## Init tau
        self.tau = float('+inf')
        ## Init A0 (threshold of significant features)    
        self.A0 = 0
        ## Project action set 
        self.action_set_proj = self.action_set @ (self.P0 @ self.D0).T
    ...
    def run(self, n=100, time_stop=20):
        ...
        while self.t < n:
            ...
            if self.t == self.u**self.epoch:
                ...
                ## reconstruct D0
                self.D0 = np.zeros(self.d) + 1
                indices_D = self.rng.choice(self.d - self.S_size, self.q)
                for i in indices_D:
                    self.D0[self.S_size + i] = np.sqrt(self.m/self.q)
                self.D0 = np.diag(self.D0)
                ## reconstruct P0
                P = self.rng.normal(0, np.sqrt(1 / self.m), size = (self.m, self.d - self.S_size))
                ...
                # Permutate features of actions et
                action_set_ = self.action_set.copy()
                for i, j_top_feat in enumerate(self.S):
                    action_set_[:, i], action_set_[:, j_top_feat] = action_set_[:, j_top_feat], action_set_[:, i]

                # Project action set with new P0 and D0
                self.action_set_proj = action_set_ @ (self.P0 @ self.D0).T

            # Draw random variable 
            t = self.t
            t+=1
            P_c0 = min(1, self.c0 * (t)**(-self.c1))
            r_t = self.rng.binomial(1, P_c0)
            y_t = 0
            ## Random sample
            if r_t == 1:
        ...
        print(f'_____Number of significant features: |S|={self.S_size}_____')
